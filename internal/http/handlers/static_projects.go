...
func WireStaticProjectRoutes(protected *gin.RouterGroup, handler *StaticProjectsHandler) {
	...
	sp.GET("", handler.List)
	sp.GET("/:id", handler.Get)
	sp.POST("/deploy", handler.Deploy)
	sp.POST("/:id/redeploy", handler.Redeploy)
	sp.PATCH("/:id/status", handler.ToggleStatus)
	sp.DELETE("/:id", handler.Delete)
	sp.GET("/:id/available-domains", handler.AvailableDomains)
	sp.PATCH("/:id/domain", handler.AssignDomain)
	sp.DELETE("/:id/domain", handler.UnassignDomain)
	sp.POST("/:id/domain/check-ip", handler.CheckDomainIP)
	sp.POST("/:id/domain/active-ssl", handler.ActiveSSL)
}
